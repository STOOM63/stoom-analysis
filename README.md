...
**Commerce Superintelligence Engine — version 2.0.0**
...
| Ventes | `Date`, `Code article`, `Vente TTC` et un identifiant `Ticket`, `Facture` ou `Num. vente` | Historique fusionné, enrichi et dédupliqué |
...
### Exports de ventes multiples

Les noms `Ventes(1)`, `Ventes(2)` ou `Ventes-juillet` ne désignent pas des formats imposés : ce sont simplement des exports différents. ANALYSIS accepte tous les exports de ventes reconnus, quelle que soit leur période ou leur nom.

Lorsque deux fichiers se recouvrent, le moteur :

- conserve les vraies lignes distinctes d'un même panier ;
- reconnaît les lignes déjà présentes ;
- évite de doubler le chiffre d'affaires ;
- enrichit les lignes existantes avec le vendeur, le numéro de vente, le marqueur de retour ou la désignation complète lorsqu'un export plus détaillé est ajouté.
...
- classement interne de contribution et de régularité, traduit dans l’interface par des statuts clairs : Produit essentiel, Vente régulière, Vente irrégulière, Saisonnier, Rupture récente, Surstock, Dormant ou Stock immobilisé ;
- produits stars, produits trafic et marge à exploiter ;
- GMROI approché et marge par euro immobilisé.

### Clients et revisite

- acheteurs uniques identifiés et tickets uniques ;
- tickets anonymes isolés pour ne pas fausser la revisite ;
- âge moyen, âge médian, tranches d’âge, villes et zone de chalandise ;
- liste cliquable de chaque client à risque ou non revenu ;
- explication du risque : retard sur le rythme habituel, absence de deuxième achat, panier en baisse, fréquence ralentie ou part de matériel modifiée ;
- historique HT/TTC, marge HT, panier HT/TTC et habitudes d’achat ;
- détection des changements de consommation et besoins récurrents ;
...
- l’import conjoint de plusieurs exports de ventes, leur enrichissement et leur déduplication ;
